...
const releaseConfigSettings = require('./assets/launcher-config.json')
const { resolveGitHubRepository: resolveGitHubRepositorySettings } = require('./assets/js/releaseconfig')
...
// Bind the add server-local account button.
document.getElementById('settingsAddLocalAccount').onclick = () => {
...
function processLogOut(val, isLastAccount){
    ...
    if(targetAcc?.type === 'local') {
        AuthManager.removeLocalAccount(uuid).then(() => {
...
const settingsCurrentLocalAccounts = document.getElementById('settingsCurrentLocalAccounts')
...
function populateAuthAccounts(){
    ...
    authKeys.forEach((val) => {
        ...
        localAuthAccountStr += accHtml

    })

    settingsCurrentLocalAccounts.innerHTML = localAuthAccountStr
}
...
function parseModulesForUI(mdls, submodules, servConf){
    ...
    for(const mdl of mdls){
        ...
        if(mdl.rawModule.type === Type.ForgeMod || mdl.rawModule.type === Type.LiteMod || mdl.rawModule.type === Type.LiteLoader || mdl.rawModule.type === Type.FabricMod || mdl.rawModule.type === Type.NeoForgeMod){
...
function populateReleaseNotes(){
    const repository = resolveGitHubRepositorySettings(releaseConfigSettings)
    if(repository == null) {
        settingsAboutChangelogText.innerHTML = Lang.queryJS('settings.about.releaseNotesFailed')
        return
    }
    $.ajax({
        url: repository.atomUrl,
        success: (data) => {
