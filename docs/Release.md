...
1. Create a GitHub repository owned by the project.
2. Add it as the `origin` remote. Keep `upstream` for optional Helios source
   comparisons.
3. Replace the package repository, homepage, issue tracker, and README download
   links with the project repository.
4. Set `updateRepository` to the HTTPS repository URL, for example
   `https://github.com/OWNER/REPOSITORY`.
5. Configure a Windows release workflow to build and attach the installer,
   blockmap, and `latest.yml` to each versioned GitHub Release.
6. Configure Windows code signing before distributing the installer publicly.
